...
const AuthForm = () => {
    ...
    return (
        <div className="w-[350px]">
            <OTPDialog showOTP={showOTP} onClose={() => setShowOTP(false)} onSuccess={handleOTPSuccess} />
            <AuthFormHeader />
...
const OTPDialog = ({ showOTP, onClose, onSuccess }: { showOTP: boolean, onClose: () => void, onSuccess: () => void }) => (
    <ResponsiveDialog
        showDialog={showOTP}
...
const AuthFormFields = ({ form, isLoading, onSubmit }: { form: any, isLoading: boolean, onSubmit: (data: any) => void }) => (
    <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <Button type="submit" className="w-full" disabled={isLoading}>
                {isLoading ? <span className="flex items-center gap-2"><Loader2 className="w-4 h-4 animate-spin" />Signing in...</span> : "Sign In"}
            </Button>
        </form>
    </Form>
)
